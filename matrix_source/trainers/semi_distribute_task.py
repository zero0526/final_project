...
class GRUPPOSCAFFOLDREPStrategy(AlgorithmStrategy):
    def __init__(self):
        super().__init__()
    ...
    def initialize_agents(self, trainer):
        ...
        trainer.shared_lower_agent = SequentialGRU_PPOAgent(
            ...
            entropy_coef=0.01,
            num_instances=trainer.num_services,
            device=trainer.device
        )

        if self.lower_mf_prev is None:
            self.lower_mf_prev = torch.zeros(
                trainer.num_services, trainer.num_nodes, lower_mf_dim,
                device=trainer.device
            )
    ...
    def _compute_node_wise_mfs(self, trainer, t_idx, s_idx, n_idx, m_idx,
                               src_node_indices):
        B = len(t_idx)
        ...
        group_sums_sn = torch.zeros(num_services, num_nodes, mf_dim,
                                    device=device)
        group_counts_sn = torch.zeros(num_services, num_nodes,
                                      device=device)

        service_node_flat_idx = s_idx * num_nodes + src_node_indices
        group_sums_sn.view(-1, mf_dim).index_add_(
            0, service_node_flat_idx.long(), two_hot
        )
        group_counts_sn.view(-1).index_add_(
            0, service_node_flat_idx.long(),
        ...
        node_wise_mfs = (
            (group_sums_s.unsqueeze(1) - group_sums_sn)
            / denom.unsqueeze(2)
        )
    ...
    def _process_lower_slot(self, trainer, obs_lower, t_idx, s_idx,
        ...
        Dùng chung cho cả training và evaluation.

        Returns: (results, final_n_idx_env, final_m_idx_env)
        """
        device = trainer.device
        agent = trainer.shared_lower_agent
        max_models = trainer.max_models

        # ── [OPT-4] Bỏ qua slot rỗng ngay lập tức ──
        if len(t_idx) == 0:
            trainer.env.time_manager.tick()
            return None, None, None

        # ── Sort tasks theo (service, node_priority, deadline) ──
        ...
        node_priority = torch.zeros(
            trainer.num_nodes, dtype=torch.long, device=device
        )
        node_priority[unique_src_nodes] = torch.arange(
            len(unique_src_nodes), device=device
        )

        sort_key = (
            s_idx.long() * 1000000
            + node_priority[src_node_indices_all] * 1000
        ...
        # ── Group by service ──
        unique_serv_sorted, counts = torch.unique(
            s_idx_sorted, return_counts=True
        )
        num_active_serv = len(unique_serv_sorted)
        max_seq_len = counts.max().item()
        offsets = torch.zeros(
            num_active_serv + 1, dtype=torch.long, device=device
        )
        offsets[1:] = torch.cumsum(counts, dim=0)

        # ── [OPT-5] Batch init: hidden, workload, masks ──
        h_dim = agent.actor.gru_cell.hidden_size

        # [OPT-5a] Loại bỏ numpy — giữ nguyên torch tensor
        h_gru_batch = torch.zeros(num_active_serv, h_dim, device=device)

        workload_sent_batch = torch.zeros(
            num_active_serv, trainer.num_nodes, device=device
        )

        # [OPT-5b] Cache placement masks
        mask_s_node_batch = torch.stack([
            self._get_service_mask(trainer, int(s))
            for s in unique_serv_sorted
        ])
        mask_s_action_batch = mask_s_node_batch.repeat_interleave(
            max_models, dim=1
        )

        # ── [OPT-6] Lọc bỏ service không có placement ──
        valid_mask = mask_s_node_batch.sum(dim=1) > 0

        if not valid_mask.all():
            # Lọc tasks thuộc service không placed
            unplaced_ids = unique_serv_sorted[~valid_mask]
            keep = ~torch.isin(s_idx_sorted, unplaced_ids)

            if keep.sum() == 0:
                trainer.env.time_manager.tick()
                return None, None, None

            t_idx_sorted = t_idx_sorted[keep]
            s_idx_sorted = s_idx_sorted[keep]
            batch_sizes_sorted = batch_sizes_sorted[keep]
            task_reqs_sorted = task_reqs_sorted[keep]
            src_node_sorted = src_node_sorted[keep]
            original_indices = sorted_indices[keep]

            h_gru_batch = h_gru_batch[valid_mask]
            workload_sent_batch = workload_sent_batch[valid_mask]
            mask_s_node_batch = mask_s_node_batch[valid_mask]
            mask_s_action_batch = mask_s_action_batch[valid_mask]
            unique_serv_sorted = unique_serv_sorted[valid_mask]

            # Recompute grouping
            counts = torch.stack([
                (s_idx_sorted == s).sum() for s in unique_serv_sorted
            ])
            max_seq_len = counts.max().item()
            offsets = torch.zeros(
                len(unique_serv_sorted) + 1, dtype=torch.long, device=device
            )
            offsets[1:] = torch.cumsum(counts, dim=0)
        else:
        ...
        # ── [OPT-7] Khởi tạo sim_backlog/capacity 1 lần ──
        ...
        # ── [OPT-8] Pre-allocate cho add_step batch ──
        if store_buffer:
            agent.memory.start_episode()
            # Buffer tạm — accumulate rồi insert
            _buf_states = {int(s): [] for s in unique_serv_sorted}
            _buf_hiddens = {int(s): [] for s in unique_serv_sorted}
            _buf_actions = {int(s): [] for s in unique_serv_sorted}
            _buf_logprobs = {int(s): [] for s in unique_serv_sorted}
            _buf_mfs = {int(s): [] for s in unique_serv_sorted}

        # ════════════════════════════════════════════
        # SEQUENTIAL LOOP — core GRU logic
        # ════════════════════════════════════════════
        for k in range(max_seq_len):
            ...
            # [OPT-2] Tensor lookup thay Python dict
            ...
            sim_backlog.index_put_(
                (n_k, curr_s.long()), load_q, accumulate=True
            )
            workload_sent_batch[active, n_k] += wl

            # [OPT-8] Accumulate buffer (không gọi add_step từng task)
            if store_buffer:
                for i, sid in enumerate(curr_s):
                    s_val = int(sid.item())
                    _buf_states[s_val].append(state_batch[i])
                    _buf_hiddens[s_val].append(new_h[i])
                    _buf_actions[s_val].append(a_ids[i].item())
                    _buf_logprobs[s_val].append(lp[i].item())
                    _buf_mfs[s_val].append(mf_input[i])

            final_n_idx[batch_idx] = n_k
            final_m_idx[batch_idx] = m_k

        # ── [OPT-8] Bulk insert vào buffer ──
        if store_buffer:
            for s_val in _buf_states:
                if len(_buf_states[s_val]) == 0:
                    continue
                tr = agent.memory.current_trajectory[s_val]
                tr['states'] = _buf_states[s_val]
                tr['hiddens'] = _buf_hiddens[s_val]
                tr['actions'] = _buf_actions[s_val]
                tr['log_probs'] = _buf_logprobs[s_val]
                tr['mfs'] = _buf_mfs[s_val]

        # ── Scatter kết quả về đúng vị trí gốc ──
        final_n_idx_env = torch.zeros(
            len(t_idx), dtype=torch.long, device=device
        )
        final_m_idx_env = torch.zeros(
            len(t_idx), dtype=torch.long, device=device
        )
        ...
        curr_slot_mfs = self._compute_node_wise_mfs(
            trainer, t_idx_sorted, s_idx_sorted,
        ...
        # ── Store end_episode + reward ──
        if store_buffer:
            true_reward = results['reward'] - results['obs']['virtual_drift']
            norm_rew = log_transform(
                true_reward
                / (trainer.config.norm_lower_rw
                   if trainer.config.norm_lower_rw != 0 else 1.0)
            )
            mf_dim = trainer.num_nodes + trainer.max_models
            for s_id in unique_serv_sorted:
                sid_val = int(s_id.item())
                if len(_buf_states.get(sid_val, [])) == 0:
                    continue
                mask_node = self._get_service_mask(trainer, sid_val)
                mask_action = mask_node.repeat_interleave(max_models)
                q_final = sim_backlog[:, sid_val]
                f_final = sim_capacity[:, sid_val] * mask_node
                wl_final = workload_sent_batch[
                    list(unique_serv_sorted).index(s_id)
                ]
                mean_mf = curr_slot_mfs[sid_val].mean(dim=0)
                gs = self._build_critic_global_state(
                    trainer, mask_node, q_final, f_final, wl_final, mean_mf
                )
                dummy_mf = torch.zeros(mf_dim, device=device)
                agent.memory.end_episode(
                    sid_val, dummy_mf, mask_action, gs, reward=norm_rew
                )
