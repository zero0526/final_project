...
def deplete_float_queue(backlog, deadline, wait_time_queue, terminal_queue, src_node_mapping, cpu_alloc, slot_duration):
    ...
    # 2. CHỈ xác định Task thành công (Hoàn thành trong slot này VÀ chưa hết hạn)
    success_mask = (backlog > 0) & (time_to_finish <= slot_duration) & (time_to_finish <= deadline)

    # 3. Cập nhật backlog: Xóa các task thành công
    new_backlog = backlog.clone()
    new_backlog[success_mask] = 0

    # 4. Xử lý task đang dở dang (pending) - Gọt bớt workload theo CPU còn dư
    ...
    # 5. Tính toán lượng xử lý (Workload Delta)
    workload_delta = backlog - new_backlog

    # 6. Phân tách Local vs External
...
def age_and_clean_dual_queue(backlog, deadline, slot_duration, *aux_queues):
    ...
    LƯU Ý: Đã bỏ tham số in_slot_violation_mask vì không cần thiết nữa.
    """
    ...
    if len(aux_queues) > 0:
        ...
        failed_terminal_ids = term_queue[true_violation_mask]
